...
- до 3 баллов: реализована какая-то нетривиальная функция score (например, использующая tf-idf или word2vec по заголовкам
  и запросам — в идеале взвешенную сумму (линейную регрессию) подобных признаков)
...
Анализ:
- до 4 баллов: вы реализовали замер качества (precision@k, MRR, DCG, pFound…) и подбираете функцию score
    хотя бы не совсем на глазок (достаточно вручную разметить несколько десятков примеров)
